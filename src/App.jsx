...
async function loadAllProjects() {
  ...
  return projects.map(p => {
    ...
    return {
      ...
      activity: pActivity.map(a => ({ initials: a.initials, text: a.text, daysAgo: Math.floor((Date.now() - new Date(a.created_at)) / 86400000) })),
    };
  });
}
...
function ProjectDetail({ project, onBack, onUpdate }) {
  ...
  const detailTabs = ["summary", "overview", "requirements", "vendors", "sourcing", "timeline", "approvals", "activity"];
        ...
        <div>
          {project.activity.map((a, i) => (
            <div key={i} style={{ display: "flex", gap: 12, padding: "12px 0", borderBottom: `1px solid rgba(255,255,255,0.04)` }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: "rgba(74,144,217,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: C.blue, flexShrink: 0, fontFamily: "monospace" }}>
                {a.initials}
              </div>
              <div>
                <div style={{ fontSize: 13, color: C.text, lineHeight: 1.5 }}>{a.text}</div>
                <div style={{ fontSize: 11, color: C.muted, marginTop: 3, fontFamily: "monospace" }}>{updatedLabel(a.daysAgo)}</div>
              </div>
            </div>
          ))}
        </div>
